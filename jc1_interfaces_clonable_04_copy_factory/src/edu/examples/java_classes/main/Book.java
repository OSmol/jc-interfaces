...
public class Book {
	...
	public static Book newInstance(Book other) {
		List<Author> newListAuthors = new ArrayList<>();
		
		for(Author a: other.getAuthors()) {
			newListAuthors.add(Author.newInstance(a));
		}
		
		return new Book(other.title, newListAuthors, other.price, other.yearPublishing);
	}
